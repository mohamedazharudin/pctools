...
export default function ResumeBuilder() {
 const { t } = useTranslation('resumeBuilder');
  ...
  return (
    <div className="max-w-6xl mx-auto text-left space-y-6 sm:space-y-8 overflow-hidden">
      {/* Tool Container */}
      <div className="flex flex-col lg:flex-row gap-5">
        
        {/* FORM CONTROLS */}
        <div className="w-full lg:flex-1 bg-slate-900 p-4 sm:p-5 rounded-2xl shadow-xl border border-slate-800 text-white">
          <h2 className="text-white text-lg sm:text-xl font-bold mt-0">📄 {t('title', 'Resume Builder')}</h2>
          <p className="text-slate-400 text-xs sm:text-sm mb-4">{t('subtitle', 'Fill out your details to generate your resume instantly.')}</p>
          <hr className="border-t border-slate-800 my-4" />

          <h3 className="font-semibold text-slate-200 my-2 text-sm sm:text-base">{t('personalInfoTitle', 'Personal Information')}</h3>
          ...
          <h3 className="font-semibold text-slate-200 my-2 text-sm sm:text-base">{t('skillsTitle', 'Skills')}</h3>
          <div className="flex gap-2 mb-3">
            <input className="flex-1 min-w-0 p-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white text-sm focus:outline-none focus:border-blue-500" value={skillInput} onChange={(e) => setSkillInput(e.target.value)} placeholder={t('placeholders.skill', 'Add a skill')} />
            <button className="px-3.5 sm:px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-500 transition-all cursor-pointer shrink-0" onClick={addSkill}>{t('addBtn', 'Add')}</button>
          </div>

          <h3 className="font-semibold text-slate-200 my-2 text-sm sm:text-base">{t('experienceTitle', 'Experience')}</h3>
          ...
          <h3 className="font-semibold text-slate-200 my-2 text-sm sm:text-base">{t('educationTitle', 'Education')}</h3>
          ...
          <br /><br />
          <button className="w-full p-3 bg-emerald-600 text-white rounded-xl text-sm sm:text-base font-bold hover:bg-emerald-500 transition-all cursor-pointer" onClick={() => window.print()}>
        ...
        <div className="w-full lg:flex-1 overflow-x-auto">
          <div id="printable-resume" className="bg-white p-5 sm:p-10 rounded-2xl shadow-xl border border-slate-200 min-h-[500px] text-slate-900 break-words">
            
            {/* Header Section */}
            <div className="mb-6">
              <h1 className="m-0 mb-2 text-slate-900 text-2xl sm:text-3xl font-bold break-words">{personal.name || t('previewDefaultName', 'Your Name')}</h1>
              <h3 className="text-blue-600 m-0 mb-2 text-base sm:text-lg font-semibold break-words">{personal.title || t('previewDefaultTitle', 'Job Title')}</h3>
              <p className="text-xs sm:text-sm text-slate-500 m-0 break-all">{personal.email} | {personal.phone}</p>
              {personal.summary && <p className="text-xs sm:text-sm text-slate-700 mt-3 leading-relaxed break-words">{personal.summary}</p>}
            </div>

            <hr className="border-t-2 border-slate-200 my-4 sm:my-6" />

            {/* Skills Section */}
            <div className="mb-6 sm:mb-8">
              <h3 className="text-slate-800 mb-3 uppercase text-sm sm:text-base tracking-wider font-semibold">{t('skillsTitle', 'Skills')}</h3>
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {skills.map((skill, i) => (
                  <span key={i} className="bg-slate-100 text-slate-700 px-2.5 py-1 rounded-lg text-xs border border-slate-200 flex items-center max-w-full break-all">
                    {skill} <b className="remove-btn text-red-500 ml-1.5 cursor-pointer shrink-0" onClick={() => removeSkill(i)}>×</b>
                  </span>
                ))}
              </div>
            </div>
            ...
            {/* Experience Section */}
            <div className="mb-6 sm:mb-8">
              <h3 className="text-slate-800 mb-3 sm:mb-4 uppercase text-sm sm:text-base tracking-wider font-semibold">{t('previewExperienceTitle', 'Work Experience')}</h3>
              {experiences.map((exp, i) => (
                <div key={i} className="mb-4 sm:mb-5">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline gap-0.5">
                    <strong className="text-xs sm:text-sm text-slate-900 font-bold break-words">{exp.role}</strong>
                    <small className="text-slate-500 font-bold text-xs shrink-0">{exp.duration}</small>
                  </div>
                  <div className="text-blue-600 text-xs sm:text-sm mb-1.5 break-words">{exp.company}</div>
                  <p className="text-xs sm:text-sm m-0 text-slate-700 leading-relaxed break-words">{exp.details}</p>
                </div>
              ))}
            </div>
            ...
            {/* Education Section */}
            <div className="mb-5">
              <h3 className="text-slate-800 mb-3 sm:mb-4 uppercase text-sm sm:text-base tracking-wider font-semibold">{t('educationTitle', 'Education')}</h3>
              {educations.map((edu, i) => (
                <div key={i} className="mb-4">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline gap-0.5">
                    <strong className="text-xs sm:text-sm text-slate-900 font-bold break-words">{edu.degree}</strong>
                    <small className="text-slate-500 font-bold text-xs shrink-0">{edu.year}</small>
                  </div>
                  <div className="text-slate-600 text-xs sm:text-sm break-words">{edu.school}</div>
                </div>
              ))}
            </div>
      ...
      <div className="p-4 sm:p-6 bg-slate-900 border border-slate-800 rounded-2xl text-slate-300 text-xs sm:text-sm leading-relaxed space-y-4 sm:space-y-6">
        <section>
          <h3 className="text-base sm:text-lg font-bold text-white mb-2">{t('aboutTitle', 'About the Online Resume Builder')}</h3>
          <p>
        ...
        <section>
          <h3 className="text-sm sm:text-base font-semibold text-white mb-2">{t('howToTitle', 'How to Build Your Resume')}</h3>
        ...
        <section>
          <h3 className="text-sm sm:text-base font-semibold text-white mb-2">{t('faqTitle', 'Frequently Asked Questions')}</h3>
